...
class AdminUserControllerTest extends AbstractControllerTest {
    ...
    void getAll() throws Exception {
        perform(MockMvcRequestBuilders.get(REST_URL))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(USER_MATCHER.contentJson(admin, user, userSecond));
    }
    ...
    void createDuplicate() throws Exception {
        User expected = new User(null, "New", FIRST_USER_MAIL, "newPass", Role.USER, Role.ADMIN);
        perform(MockMvcRequestBuilders.post(REST_URL)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void enable() throws Exception {
        perform(MockMvcRequestBuilders.patch(REST_URL + FIRST_USER_ID)
                .param("enabled", "false")
                .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isNoContent());

        assertFalse(userRepository.getById(FIRST_USER_ID).isEnabled());
    }
